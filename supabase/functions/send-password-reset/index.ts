...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .title { font-size: 20px; font-weight: 600; color: #1a1a1a; margin-bottom: 20px; }
            .message { font-size: 16px; color: #4a5568; line-height: 1.6; margin-bottom: 30px; }
            .button { display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; text-decoration: none; padding: 14px 40px; border-radius: 8px; font-weight: 600; font-size: 16px; }
    ...
    const emailResponse = await resend.emails.send({
      ...
      to: [user.email],
      subject: "Reset your Wanderer password",
      html: emailHtml,
    });
